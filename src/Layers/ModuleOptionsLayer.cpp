...
void ModuleOptionsLayer::customSetup() {
    ...
    for (size_t i = 0; i < mod->options.size(); i++) {
        CCPoint pos = ccp(this->size.x / 2 + (sizee.x * (x == 0 ? -1 : 0)), this->size.y - 50 - (sizee.y * y)) +
                      ccp(15 + 7.5f, 0);
    ...
    auto about = CCMenuItemSpriteExtra::create(CCSprite::createWithSpriteFrameName("GJ_infoIcon_001.png"),
                                               this,
                                               menu_selector(ModuleOptionsLayer::onInfoButton));

...
void EditLabelPopup::customSetup() {
    ...
    auto anchorInfo = InfoAlertButton::create(
        "Anchor Info", "Anchor is the <cc>corner</c> of the screen that <cl>labels</c> are anchored to", 0.4f);
    anchorInfo->setPosition(ccp(333, 208));

    auto offsetInfo = InfoAlertButton::create(
        "Offset Info",
        "Offset in <cl>cocos units</c> from the anchor point, will not affect other labels in layout",
        0.4f);
    offsetInfo->setPosition(ccp(330, 78));

    auto nameInfo = InfoAlertButton::create(
        "Display Name", "A name for your label, only shown in the <cc>ui</c> to differentiate your labels", 0.4f);
    ...
    dropdown =
        Dropdown::create(ccp(rightBG->getContentWidth() - 10, 20), strs, menu_selector(EditLabelPopup::onAddEvent));

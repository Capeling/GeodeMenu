...
#pragma region Setup Windows
    ...
    static void SetupLevel() {
        ...
        level->modules.push_back(new Module(
            "Instant Complete",
            "instant",
            "Instantly completes a level.\nMay not work because of the <cg>Geometry Dash</c> "
            "anti-cheat.\nUsing this "
            "cheat on a <co>rated level</c> <cr>WILL GET YOU LEADERBOARD BANNED</c>",
            false,
            "<cr>Warning!</c>\nUsing this cheat on a <co>rated level</c> <cr>WILL GET YOU "
            "LEADERBOARD BANNED</c>",
            true
        ));
        ...
        level->modules.push_back(new Module(
            "Smart StartPos",
            "smart-startpos",
            "Automatically adjusts the settings of a startpos to correctly set the gamemode, "
            "speed, and more",
            true
        ));
        ...
        level->modules.push_back(new Module(
            "Accurate Percentage", "accurate-percentage", "Allows you to have any amount of decimal places in your percentage text"
        ));
        level->modules.push_back(new Module(
            "Classic Percentage",
            "classic-percentage",
            "Percentage is calculated like in 2.1, The percentage for new best is still calculated "
            "like 2.2"
        ));
        ...
        level->modules.push_back(
            new Module("Conditional Auto-Retry", "conditional-retry", "Shows the retry level popup ONLY when you get a new best")
        );
        level->modules.push_back(
            new Module("Auto Practice Mode", "auto-practice", "Automatically puts you into practice mode when you open a level")
        );
        ...
        level->modules.push_back(
            new Module("No Checkpoint Limit", "no-checkpoint-limit", "Removes the 50 checkpoint limit from practice mode")
        );
        ...
        level->modules.push_back(
            new Module("Allow Pause Buffering", "pause-buffering", "Removes the fix for pause buffering (the old sakupen circles thing)")
        );
        ...
        level->modules.push_back(
            new Module("Icon Kit in Pause", "pause-icon-kit", "Adds a button to open the icon kit in the pause menu", true)
        );

        // level->modules.push_back(new Module("Gamemode Switcher", "gamemode-switcher", "Adds a
        // button to the bottom of the pause menu to change your gamemode"));
        ...
        Client::GetModule("noclip")->options.push_back(
            new Module("Minimum Accuracy", "noclip-min-accuracy-toggle", "Allows you to die if your noclip accuracy drops below this value")
        );
        ...
        Client::GetModule("show-layout")
            ->options.push_back(new Module("Keep Camera Triggers", "layout-retain-camera", "Keeps the 2.2 camera triggers in the level", true)
            );
        ...
        Client::GetModule("frame-stepper")
            ->options.push_back(new Module("unused", "unused-module", "this is here because i need a module for the popup"));
        // Client::GetModule("smart-startpos")->options.push_back(new SmartStartposUIModule());
        // Client::GetModule("smart-startpos")->optionSizeForce =
        // Client::GetModule("smart-startpos")->options[0]->sizeForOptionsPage();
    ...
    static void SetupBypass() {
        ...
        // bypass->modules.push_back(new Module("Slider Limit Bypass", "slider-limit", "Allows sliders
        // to go beyond the limit of the slider. <cr>Doesn't work for scaling in the editor currently</c>"));
    ...
    static void SetupUniversal() {
        ...
        bypass->modules.push_back(
            new Module("Practice Music Sync", "practice-bypass", "Unlocks the Practice Music Sync button in the pause menu options")
        );
        ...
        bypass->modules.push_back(
            new Module("Fire in the hole!", "fire-in-the-hole", "Replaces every single sfx with\n<cr>FIRE IN THE HOLE!</c>")
        );

        bypass->modules.push_back(
            new Module("Auto Safe mode", "auto-safe-mode", "Automatically enables safe mode if you have cheats enabled", true)
        );
        ...
        bypass->modules.push_back(new Module(
            ...
            false,
            "<cr>Warning!</c>\nThis mod has been <cl>known</c> to cause issues on <cg>some</c> "
            "devices such "
        ...
        bypass->modules.push_back(
            new Module("Full Options Menu", "full-options", "Replaces the mini options menu in the pause menu with the full options menu")
        );

        bypass->modules.push_back(new Module("Physics Bypass", "tps-bypass", "Allows your physics to run at values other than 240"));
        bypass->modules.push_back(new Module(
            "Comment History Bypass", "comment-history", "Allows you to view the comment history of any user, rob still hasn't fixed this"
        ));
        ...
        bypass->modules.push_back(
            new Module("No Short Numbers", "no-short-nums", "Show the full value instead of the short value in level cells")
        );
        ...
        bypass->modules.push_back(new Module(
            "Treasure Room Bypass", "treasure-room-bypass", "Allows you to enter the treasure room without having <cc>5</c> demon keys"
        ));
        ...
        bypass->modules.push_back(new Module(
            "The Challenge Bypass", "the-challenge-bypass", "Allows you to enter the vault of secrets without having <cc>50</c> diamonds"
        ));

        bypass->modules.push_back(new Module(
            "Basement Bypass", "basement-bypass", "Allows you to enter the monsters basement without completing <cc>The Challenge</c>"
        ));
        bypass->modules.push_back(
            new Module("Basement Key Bypass", "basement-key-bypass", "Allows you to unlock the keys in the monsters basement")
        );

        bypass->modules.push_back(
            new Module("Treasure Room Chest Bypass", "chest-unlock-bypass", "Allows you to open any chest in the <cc>treasure room</c>")
        );
        bypass->modules.push_back(new Module("Unlock All Gauntlets", "unlock-gauntlets", "Allows you to open any level in the gauntlet"));

        bypass->modules.push_back(
            new Module("December Menu Snow", "december-snow", "Shows snow on the main menu during <cc>december</c>.", true)
        );
        bypass->modules.push_back(new Module(
            "End Screen Cheat Indicator", "end-screen-cheat-indicator", "Shows the cheat indicator on the <cc>end screen</c>.", true
        ));
        Client::instance->windows.push_back(bypass);
        ...
        // Client::GetModule("custom-trans")->options.push_back(new InputModule("Time Mod (*)",
        // "transition-time-modifier", "1.0")); Client::GetModule("custom-trans")->options.push_back(new
        // ColourModule("Fade Colour:", "transition-fade-colour", ccc3(0, 0, 0)));
    ...
    static void SetupCreator() {
        ...
        creator->modules.push_back(
            new Module("Copy Hack", "copy-hack", "Allows you to copy any level from the servers\nCode by <co>Firee</c>")
        );
        creator->modules.push_back(new Module(
            "Show Level Password", "show-password", "Adds a button to the password input popup that shows the level password", true
        ));

        creator->modules.push_back(
            new Module("No Copy Mark", "noc-hack", "Hides the (c) mark from your levels on publish.\nCode by <co>Firee</c>")
        );
        ...
        creator->modules.push_back(
            new Module("Smooth Editor Trail", "smooth-editor-trail", "Updates the editor trail at your screen refresh rate instead of 30 fps")
        );

        creator->modules.push_back(
            new Module("Editor Extension", "editor-extension", "Increases the length of the editor by <cs>17895</c> times :3")
        );
        creator->modules.push_back(new Module(
            "Toolbox Button Bypass", "toolbox-buttons", "Unlimited <cc>Rows</c> and <cc>Columns</c> in your <cl>Object Toolbox</c>"
        ));
    ...
    static void SetupCosmetic() {
        ...
        // cosmetic->modules.push_back(new Module("Hide Endscreen BTN", "end-screen", "Adds an arrow to hide the end
        // screen"));
        ...
        cosmetic->modules.push_back(
            new Module("Pulse Menu", "menu-pulse", "Pulses the <cg>Geometry Dash</c> logo on the main menu to the music")
        );
        cosmetic->modules.push_back(new Module(
            "No \"Do Not\" flip",
            "just-dont",
            "Disables the attempt text and endscreen text from being flipped with the <cl>Do "
            "Not</c> option enabled."
        ));

        // cosmetic->modules.push_back(new Module("Force Don't Fade", "dont-fade", "Forces all
        // object to be Don't Fade")); cosmetic->modules.push_back(new Module("Force Don't Enter",
        // "dont-enter", "Forces all object to be Don't Enter"));
...
// cosmetic->modules.push_back(new Module("No Player Rotation", "no-rot", "Disables rotation on players"));
        ...
        cosmetic->modules.push_back(
            new Module("Unlock Buttons", "unlock-buttons", "Visually Unlocks the greyed out buttons in the online page")
        );
        cosmetic->modules.push_back(
            new Module("No Wave Reset", "no-wave-reset", "Stops the wave trail from disappearing when you change gamemode")
        );

        cosmetic->modules.push_back(
            new Module("No Orb Pulse", "no-orb-pulse", "Disables orb's from pulsing, Doesn't work on main levels made before 2.2")
        );
        cosmetic->modules.push_back(new Module(
            "Main Menu Gameplay",
            "main-menu-gameplay",
        ...
        cosmetic->modules.push_back(
            new Module("Hide Pause Button", "hide-pause-button", "Hides the pause button in game, requires reopening level to apply")
        );
        cosmetic->modules.push_back(
            new Module("Coins In Practice", "practice-coins-visual", "Allows you to visually collect coins in practice mode")
        );
        ...
        cosmetic->modules.push_back(
            new Module("Gold User Coins", "gold-user-coins", "Makes user coins appear as if they are gold robtop coins")
        );
        cosmetic->modules.push_back(new Module("No Ship Fire", "no-ship-fire", "Hides the fire behind the ship while it's flying"));

        cosmetic->modules.push_back(
            new Module("Ball Rotation Bug", "ball-rotation-bug", "EMULATES the old rotation bug when switching gamemodes from the ball")
        );
        ...
        cosmetic->modules.push_back(
            new Module("No Plat Jump Anim", "plat-jump-anim", "Disables the squish animation when jumping in <cc>platformer mode</c>")
        );

        cosmetic->modules.push_back(new Module("No Lightning", "no-lightning", "Disables the lightning when touching certain portals"));

        // cosmetic->modules.push_back(new Module("No Camera Movement", "no-camera", "Disables camera
        // movements that are made with <cl>triggers</c>")); cosmetic->modules.push_back(new Module("No
        // Player Rotation", "no-plr-rot", "Disables Player Rotation :3\nIt looks ugly imo but you do you"));
    ...
    static void SetupMisc() {
        ...
        // misc->modules.push_back(new Module("Use Full Options", "full-options", "Opens the full
        // options menu instead of the mini options in the pause menu.\nAlso adds a button to the
        // normal pause menu to open the mini options."));
    ...
    static void SetupOptions(bool android) {
        ...
        if (true) {
            ...
            auto sbox =
                new Module("Search Box", "ui-search-box", "Adds a search box to the bottom of the ui instead of <cc>version info</c>.");
            ...
            cfg->modules.push_back(new Module(
                "Additional Border",
                "npesta-width",
                "Adds borders to the left and right sides of the mod menu to make the menu look "
                "better with "
            ...
            cfg->modules.push_back(new Module(
                "Hide Button",
                "hide-btn",
                "Hides the button to open the menu, use the <cl>Tab</c> or <cl>Insert</c> key to "
                "open the mod menu",
                true
            ));
#endif
            ...
            cfg->modules.push_back(
                new Module("Save Button Position", "save-pos", "Saves the position of the ball on screen between sessions", true)
            );
...
// Client::GetModule("custom-obj-limit")->setIncompatible("This mod only supports <cl>Windows</c> <cg>currently</c>");
        ...
        // Client::GetModule("rand-seed")->setIncompatible("This mod has <cr>not yet</c> been ported
        // to <cl>2.206</c>"); Client::GetModule("show-trajectory")->setIncompatible("This mod has
        // <cr>not yet</c> been ported to <cl>2.206</c>");
